...
import { PixPagamento } from "../criar-declaracao/components/forms-templates/pagamentos/carrinho-pagamento-pix";
...
export function ModalPagamentoPendente({ page, onFechar }: ModalPagamentoPendenteProps) {
  ...
  // ── Seção de resumo + QR (reutilizada nas duas abas) ─────────────────────
  function ResumoEMolduras() {
    return (
      <>
        {/* Resumo do pedido */}
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
          <h3 className="font-bold text-xs mb-3 text-gray-700 uppercase tracking-wide">
            Resumo do pedido
          </h3>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg max-h-[calc(100vh-100px)] overflow-y-auto shadow-2xl">
        ...
        <div className="sticky top- bg-white border-b border-gray-200 flex items-center justify-between px-4 py-3 rounded-t-2xl z-10">
          <div className="flex items-center gap-2">
            {metodo !== "selecao" && (
              <button
                onClick={() => { setMetodo("selecao"); setPaymentLink(""); setErro(""); }}
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {page.receiverName
                  ? `Declaração para ${page.receiverName}`
                  : "Declaração pendente"}
              </p>
            </div>
          </div>
        ...
        <div className="p-4 space-y-3">
          ...
          {metodo === "selecao" && (
            <>
              ...
              {/* Seus dados */}
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                <h3 className="font-bold text-xs mb-2 text-gray-700 uppercase tracking-wide">
                  Seus dados
                </h3>
                <div className="w-full px-3 py-2.5 bg-gray-100 border border-gray-200 rounded-xl text-sm text-gray-600">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-[#e687cd] bg-pink-50 hover:bg-pink-100 transition-all active:scale-[0.98]"
                  >
                    <MdPix size={28} className="text-[#e687cd]" />
                    <div className="text-center">
                      <p className="text-sm font-bold text-gray-800">PIX</p>
                      <p className="text-[11px] text-gray-500">QR Code instantâneo</p>
                    </div>
          ...
          {/* ── PIX ── */}
          {metodo === "pix" && page.id && page.planType && (
            <PixPagamento
              pageId={page.id}
              selectedPlan={page.planType}
              totalAmount={total}
              qrCodeFrame={molduraSelecionada}
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                ...
                {!paymentLink ? (
                  <button
                    ...
                    disabled={isCreating}
                    className="w-full py-3 rounded-xl font-bold text-sm bg-[#e687cd] hover:bg-pink-400 text-white transition-all active:scale-[0.98] shadow-lg shadow-pink-100 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
                  >
                    {isCreating ? (
