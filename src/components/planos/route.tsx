        ...
        <div className="bg-[#FAFAFA] min-h-screen flex justify-center items-center px-6 py-16">
            <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-12">
                {planos.map((plano, index) => (
                    <div
                        key={index}
                        className={`p-8 rounded-2xl border-2 flex flex-col gap-6 bg-white shadow-sm ${
                            plano.recomendado ? "border-[#e687cd]" : "border-gray-200"
                        }`}
                    >
                        {plano.recomendado && (
                            <span className="bg-[#e687cd] text-white px-3 py-1 rounded-full text-sm w-max">
                                ⭐ Recomendado
                            </span>
                        )}
                        <h2 className="text-2xl font-bold text-[#e687cd]">{plano.titulo}</h2>
                        ...
                        <ul className="flex flex-col gap-2">
                            {plano.recursos.map((recurso, i) => (
                                ...
                                >
                                    {recurso.ativo
                                        ? <FaCheck className="shrink-0" color="#22c55e" />
                                        : <FaTimes className="shrink-0" color="#f87171" />
                                    }
                                    {recurso.text}
                                </li>
                            ))}
